                    ...
                    <div className="order-1 lg:order-1">
                      ...
                      {(selectedSavedAddress === null || !savedAddresses.find(a => a.id === selectedSavedAddress)) && (
                        <div className="space-y-3 mt-4">
                          ...
                          <div className="mt-4 p-4 bg-[#8b5cf6]/10 border border-[#8b5cf6]/20 rounded-2xl">
                            <div className="flex items-center gap-2 mb-2">
                              <QrCode size={16} className="text-[#8b5cf6]" />
                              <span className="text-sm font-black text-white">Apply Coupon</span>
                            </div>
                            <div className="flex gap-2">
                              <input 
                                ...
                                className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-sm font-bold outline-none focus:border-[#8b5cf6]"
                              />
                              <button 
                                onClick={(e) => {
                                  e.preventDefault();
                                ...
                                className="px-4 py-2 bg-[#8b5cf6] text-white rounded-xl text-xs font-black uppercase"
                              >
                                Apply
                              </button>
                            </div>
                            {isCouponApplied && (
                              <div className="mt-2 text-xs text-green-400 font-bold">✓ SABBPE20 Applied - 20% OFF!</div>
                            )}
                          </div>
                        </div>
                      )}
                    ...
                    <div>
                      ...
                      <div className="mt-6 space-y-2">
                        ...
                        <div className="flex justify-between text-lg font-black pt-4 border-t border-white/10">
                          <span>Total</span>
                          <span>₹{isCouponApplied ? Math.round(cart.reduce((sum, item) => sum + item.price, 0) * 0.8) : cart.reduce((sum, item) => sum + item.price, 0)}</span>
                        </div>
                      </div>
                ...
                {checkoutStep === 2 && (
                  <div className="grid grid-cols-1 gap-8">
                    ...
                    <div className="space-y-6">
                      ...
                      <button
                        onClick={async () => {
                          ...
                          const newOrder = {
                            ...
                            total: isCouponApplied ? Math.round(cart.reduce((sum, item) => sum + item.price, 0) * 0.8) : cart.reduce((sum, item) => sum + item.price, 0),
                      ...
                      >
                        {isProcessingPayment ? 'Processing...' : `Pay ₹${isCouponApplied ? Math.round(cart.reduce((sum, item) => sum + item.price, 0) * 0.8) : cart.reduce((sum, item) => sum + item.price, 0)} with SabbPe`}
                      </button>
